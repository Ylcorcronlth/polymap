...
namespace Interpolate {
    public class InverseDistanceWeighting {
        ...
        private float FalloffWeight;  // The base weight used to prevent division by zero and make interpolation go to zero far from any points.

        public InverseDistanceWeighting(Vector2[] points, float search_radius=0.05f, float exponent=2.0f, float falloff_weight=1.0f) {
            Points = points;
            SearchRadius = search_radius;
            Exponent = exponent;
            FalloffWeight = falloff_weight;
        ...
        private List<int> GetNearbyPoints(Vector2 pos) {
            ...
            int xo = (pos.x*Size - xi > 0.5f ? 1 : -1);
            int yo = (pos.y*Size - yi > 0.5f ? 1 : -1);
        ...
        public float Evaluate(Vector2 pos, float[] values, Vector2[] gradients) {
            ...
            float weight_sum = FalloffWeight;
